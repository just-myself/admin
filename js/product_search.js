...
$("#product-kw").keyup(function(e){
  var l = $("#price_low").val();
  var h = $("#price_high").val();
  
  if(e.keyCode==13){
    //loadProductByPage(1, 10,this.value);
    loadProductByPageSeacrh(1,10,this.value,l,h);
  }
});
...
function loadProductByPageSeacrh(pno, pageSize,kw,low,high){
  $('#table-laptop tbody').html('<div class="loading"><img src="img/loading.gif" alt=""></div>');
  $.ajax({
    url: 'data/04_product_list.php',
    data: {pno:pno, pageSize: pageSize,kw:kw,low:low,high:high},
    success: function(pager){
      //表格内容
      var html = '';
      $.each(pager.data, function(i, l){
